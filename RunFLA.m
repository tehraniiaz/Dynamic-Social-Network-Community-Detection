...
function pop = RunFLA(pop, params)
    ...
    for it = 1:beta
        ...
        for k=1:alpha
            ...
            randPos = sort(randperm(VarSize(2),2));
            ...
            % Improvement Step 1
            % NewSol1.Position = [B(1).Position(1:randPos),B(end).Position((randPos+1):end)];
            NewSol1.Position = [B(1).Position(1:randPos(1)),B(end).Position(randPos(1)+1:randPos(2)),B(1).Position((randPos(2)+1):end)];
            ...
            % Improvement Step 2
            if ImprovementStep2
                % NewSol2.Position = [B(end).Position(1:randPos),B(1).Position((randPos+1):end)];
                NewSol2.Position = [B(end).Position(1:randPos(1)),B(1).Position(randPos(1)+1:randPos(2)),B(end).Position((randPos(2)+1):end)];
                % NewSol2 = B(end);
            ...
            % Censorship
            if Censorship
                B(end).Position = arrayfun(@(ind)randi([LowerBound(ind), UpperBound(ind)],1),1:VarSize(2));
